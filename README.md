...
union all select 'meeting_requests',    to_regclass('public.meeting_requests')  is not null;
```
...
Aparte de las migraciones hay una Edge Function. Se despliega con la CLI:

```bash
supabase functions deploy verify-receipt
supabase secrets set APPLE_SHARED_SECRET=...
supabase secrets set GOOGLE_SERVICE_ACCOUNT_JSON="$(cat cuenta-servicio.json)"
supabase secrets set ANDROID_PACKAGE_NAME=com.criadorpro.app
```

Sin esos secretos la función **rechaza todos los recibos**, a propósito: sin
forma de comprobar uno, concederlo convertiría la suscripción en un honor
system.
